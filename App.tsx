...
const App: React.FC = () => {
  ...
  if (!auth.isLoggedIn) {
    return (
      <div className="h-screen w-full flex items-center justify-center bg-slate-950 p-6 relative">
    ...
    <div className="h-screen flex flex-col md:flex-row bg-slate-50 overflow-hidden text-slate-900 font-inter">
      <aside className="w-full md:w-80 bg-slate-900 text-white flex flex-col shrink-0 z-30 shadow-2xl">
        ...
        <div className="p-8 border-t border-slate-800 space-y-3">
           ...
           <button onClick={() => window.location.reload()} className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-rose-400 hover:bg-rose-500/10 font-black text-xs uppercase transition-all"><LogOut size={18} /> Sair do Sistema</button>
        </div>
      </aside>

      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-24 bg-white border-b border-slate-200 px-6 md:px-12 flex items-center justify-between">
          <h2 className="font-black text-slate-900 uppercase text-sm md:text-lg truncate">{auth.companyName} <span className="hidden md:inline text-slate-300 mx-2">|</span> <span className="text-blue-600 font-bold">{auth.currentUser?.fullName}</span></h2>
          <div className="text-[10px] font-black uppercase bg-emerald-50 text-emerald-600 px-4 py-2 rounded-full border border-emerald-100 flex items-center gap-2 shadow-sm">
            <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div> Local Ativo
          </div>
        </header>
              ...
              <div className="bg-indigo-600/10 border border-indigo-200 p-6 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-4">
                 <div className="flex items-center gap-4">
                   ...
                   <div>
                     ...
                     <p className="text-indigo-700 text-[10px] font-medium">Seus dados estão seguros neste navegador. Use a opção "Exportar Backup" para salvar no seu PC ou GitHub.</p>
                   </div>
                 </div>
                 <button onClick={exportData} className="px-6 py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg shadow-indigo-600/20 active:scale-95 transition-all">Baixar Backup Agora</button>
              </div>
            </div>
          )}
          ...
          {activeView === 'projects' && (
            <div className="max-w-7xl mx-auto space-y-10">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  {projects.map(p => (
                    <div key={p.id} className="bg-white rounded-[3rem] border border-slate-200 p-8 md:p-10 shadow-lg relative overflow-hidden flex flex-col border-t-8 border-t-blue-600 transition-all hover:shadow-2xl">
                       <div className="absolute top-8 right-8 flex gap-2">
                          <button onClick={() => { setEditingProjectId(p.id); setProjName(p.name); setProjClient(p.client); setProjBudget(p.budget.toString()); setProjTax(p.taxPercentage.toString()); setProjCommission(p.commissionPercentage.toString()); setProjLocation(p.location || ''); setProjDate(p.startDate); setIsProjectModalOpen(true); }} className="p-3 text-slate-300 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all"><Edit2 size={18} /></button>
                          <button onClick={() => { if(window.confirm('Excluir obra permanentemente?')) setProjects(projects.filter(proj => proj.id !== p.id)) }} className="p-3 text-slate-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all"><Trash2 size={18} /></button>
                       </div>
                       <div className="flex-1">
                          <h4 className="font-black text-xl md:text-2xl uppercase mb-1 text-slate-800 truncate pr-16">{p.name}</h4>
                          ...
                          <div className="grid grid-cols-2 gap-4 mb-8">
                             <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100">
                                ...
                                <p className="font-black text-rose-600 text-base md:text-lg">{formatBRL(p.expenses.reduce((s, e) => s + e.amount, 0))}</p>
                             </div>
                             <div className="p-5 bg-emerald-50 rounded-3xl border border-emerald-100">
                                <p className="text-[9px] font-black text-emerald-600 uppercase mb-1">Medição Real</p>
                                <p className="font-black text-emerald-700 text-base md:text-lg">{formatBRL(p.revenues.reduce((s, r) => s + r.amount, 0))}</p>
                             </div>
                          </div>
                          <div className="bg-blue-50/50 rounded-[2rem] p-6 mb-8 border border-blue-100">
                             <h5 className="text-[10px] font-black uppercase text-blue-600 flex items-center gap-2 mb-4"><Clock size={14} /> Cronograma de Medições</h5>
                             <div className="space-y-2">
                                {p.plannedRevenues?.map(rev => (
                                   <div key={rev.id} className="bg-white p-4 rounded-2xl shadow-sm flex items-center justify-between border border-blue-100">
                                      <p className="font-black text-slate-800 text-xs truncate max-w-[150px] md:max-w-none">{formatBRL(rev.amount)} <span className="hidden md:inline text-slate-400 font-bold ml-2">| {rev.description}</span></p>
                                      <div className="flex gap-2">
                                        <button onClick={() => handleMarkAsPaid(p.id, rev.id)} className="p-2 bg-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-lg transition-all" title="Corrigir e Receber"><CheckCircle size={16} /></button>
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <button onClick={() => { resetTransactionForm(); setTransactionModal({ isOpen: true, projectId: p.id, transactionId: null, convertingPlannedId: null, type: 'expense' }); }} className="py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase shadow-lg transition-all active:scale-95">Lançar Custos</button>
                          <button onClick={() => { resetTransactionForm(); setTransactionModal({ isOpen: true, projectId: p.id, transactionId: null, convertingPlannedId: null, type: 'revenue' }); }} className="py-4 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-lg transition-all active:scale-95">Lançar Receita</button>
                          <button onClick={() => { resetTransactionForm(); setTransactionModal({ isOpen: true, projectId: p.id, transactionId: null, convertingPlannedId: null, type: 'planned_revenue' }); }} className="py-4 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-lg transition-all active:scale-95">Lançar Previsto</button>
                       </div>
                    </div>
                  ))}
               </div>
          ...
          {activeView === 'reports' && (
            <div className="max-w-7xl mx-auto space-y-8 animate-in slide-in-from-right-8 duration-700 pb-20">
               <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                  ...
                  <select value={reportFilterProject} onChange={e => setReportFilterProject(e.target.value)} className="bg-white border border-slate-200 p-4 rounded-xl font-black text-[10px] uppercase outline-none shadow-sm">
                    <option value="all">Filtro: Todas as Obras</option>
                    {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
               </div>
               <div className="bg-white rounded-[3rem] border border-slate-200 overflow-x-auto shadow-2xl">
                  <table className="w-full text-left min-w-[800px]">
                     <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                           <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400">Data</th>
                           <th className="px-10 py-6 text-[10px] font-black uppercase text-slate-400">Obra / Descrição</th>
                     ...
                     <tbody className="divide-y divide-slate-100">
                        {allExpensesSorted.map(e => (
                           <tr key={e.id} className="hover:bg-slate-50/50 transition-all group">
                              <td className="px-10 py-6 font-black text-slate-600 text-xs">{new Date(e.date).toLocaleDateString()}</td>
                              <td className="px-10 py-6"><p className="font-black text-slate-900 text-xs uppercase">{e.projectName}</p><p className="text-[10px] text-slate-400 font-bold italic">{e.description}</p></td>
                              <td className="px-10 py-6"><span className="px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-[9px] font-black uppercase">{e.category}</span></td>
                              <td className="px-10 py-6 text-right font-black text-rose-600">{formatBRL(e.amount)}</td>
                              <td className="px-10 py-6 text-center">
                                 <button onClick={() => openTransactionEdit(e.projectId, 'expense', e.id)} className="p-2 text-slate-300 hover:text-blue-600 group-hover:bg-blue-50 rounded-lg transition-all"><Edit2 size={14} /></button>
                              </td>
                           </tr>
                        ))}
                     </tbody>
          ...
          {activeView === 'ai' && (
            <div className="max-w-4xl mx-auto p-12 bg-white rounded-[3rem] border border-slate-200 shadow-2xl">
               <div className="flex items-center justify-between mb-8">
                  <h3 className="text-2xl font-black uppercase">Auditoria de Inteligência</h3>
                  <button onClick={async () => { setIsAiLoading(true); setAiReport(await analyzeFinancials(projects)); setIsAiLoading(false); }} className="p-4 bg-slate-100 rounded-2xl active:rotate-180 transition-all duration-500"><RefreshCw size={24} className={isAiLoading ? 'animate-spin' : ''} /></button>
               </div>
               <div className="bg-slate-50 p-10 rounded-[2rem] prose prose-slate max-w-none min-h-[300px] flex items-center justify-center">
                 {isAiLoading ? <Loader2 size={40} className="animate-spin text-blue-600" /> : <div className="w-full text-slate-700 whitespace-pre-wrap font-medium leading-relaxed">{aiReport || "Inicie a análise para auditoria profissional dos seus lucros e gastos."}</div>}
               </div>
            </div>
          )}
        </div>
      ...
      {isProjectModalOpen && (
        <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-2xl z-[100] flex items-center justify-center p-4 md:p-8">
           <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl animate-in zoom-in-95 duration-300">
              <div className="p-8 bg-blue-600 text-white flex justify-between items-center rounded-t-[3rem]">
                 <h3 className="text-xl md:text-2xl font-black uppercase tracking-tight">{editingProjectId ? 'Configurar Obra' : 'Nova Obra'}</h3>
                 <button onClick={() => setIsProjectModalOpen(false)} className="p-3 hover:bg-white/10 rounded-full transition-all"><X size={24} /></button>
              </div>
              <form onSubmit={handleSaveProject} className="p-8 space-y-6">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <input required placeholder="Identificação da Obra" value={projName} onChange={e => setProjName(e.target.value)} className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:border-blue-500 outline-none" />
                    <input required placeholder="Cliente Contratante" value={projClient} onChange={e => setProjClient(e.target.value)} className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:border-blue-500 outline-none" />
                 </div>
                 <input placeholder="Endereço / Local" value={projLocation} onChange={e => setProjLocation(e.target.value)} className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:border-blue-500 outline-none" />
                 ...
                 <button type="submit" className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl uppercase shadow-xl flex items-center justify-center gap-4 transition-all hover:scale-[1.01] active:scale-95"><Save size={24} /> Confirmar Cadastro</button>
              </form>
           </div>
        </div>
      )}
      ...
      {transactionModal.isOpen && (
         <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-2xl z-[100] flex items-center justify-center p-4 md:p-8 overflow-y-auto">
            <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl my-auto animate-in zoom-in-95 duration-300 border border-white/20">
               <div className={`p-8 text-white flex justify-between items-center rounded-t-[3rem] ${
                 ...
                 transactionModal.type === 'revenue' ? 'bg-emerald-600' : 'bg-blue-600'
                  ...
                  <button onClick={() => { setTransactionModal({ isOpen: false, projectId: null, transactionId: null, convertingPlannedId: null, type: 'expense' }); resetTransactionForm(); }} className="p-4 hover:bg-white/10 rounded-full transition-all"><X size={24} /></button>
               </div>
               ...
               <form onSubmit={handleSaveTransaction} className="p-8 space-y-6">
                  ...
                  {transactionModal.type === 'expense' && !transactionModal.transactionId ? (
                    <div className="space-y-4 pt-4 border-t border-slate-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center gap-4 group focus-within:border-blue-500 transition-all">
                          <div className="p-3 bg-blue-100 text-blue-600 rounded-xl group-focus-within:bg-blue-600 group-focus-within:text-white transition-all"><HardHat size={20}/></div>
                          <div className="flex-1">
                            <p className="text-[9px] font-black uppercase text-slate-400">Mão de Obra</p>
                            <input type="number" step="0.01" placeholder="R$ 0,00" value={costAmounts['Mão de Obra']} onChange={e => setCostAmounts({...costAmounts, 'Mão de Obra': e.target.value})} className="bg-transparent font-black text-lg w-full outline-none" />
                          </div>
                        </div>

                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center gap-4 group focus-within:border-blue-500 transition-all">
                          <div className="p-3 bg-emerald-100 text-emerald-600 rounded-xl group-focus-within:bg-emerald-600 group-focus-within:text-white transition-all"><Package size={20}/></div>
                          <div className="flex-1">
                            <p className="text-[9px] font-black uppercase text-slate-400">Material</p>
                            <input type="number" step="0.01" placeholder="R$ 0,00" value={costAmounts['Material']} onChange={e => setCostAmounts({...costAmounts, 'Material': e.target.value})} className="bg-transparent font-black text-lg w-full outline-none" />
                          </div>
                        </div>

                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center gap-4 group focus-within:border-blue-500 transition-all">
                          <div className="p-3 bg-indigo-100 text-indigo-600 rounded-xl group-focus-within:bg-indigo-600 group-focus-within:text-white transition-all"><Users size={20}/></div>
                          <div className="flex-1">
                            <p className="text-[9px] font-black uppercase text-slate-400">Serviços de Terceiros</p>
                            <input type="number" step="0.01" placeholder="R$ 0,00" value={costAmounts['Serviços de Terceiros']} onChange={e => setCostAmounts({...costAmounts, 'Serviços de Terceiros': e.target.value})} className="bg-transparent font-black text-lg w-full outline-none" />
                          </div>
                        </div>

                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center gap-4 group focus-within:border-blue-500 transition-all">
                          <div className="p-3 bg-slate-200 text-slate-600 rounded-xl group-focus-within:bg-slate-800 group-focus-within:text-white transition-all"><Receipt size={20}/></div>
                          <div className="flex-1">
                            <p className="text-[9px] font-black uppercase text-slate-400">Outras Despesas</p>
                            <input type="number" step="0.01" placeholder="R$ 0,00" value={costAmounts['Outras Despesas']} onChange={e => setCostAmounts({...costAmounts, 'Outras Despesas': e.target.value})} className="bg-transparent font-black text-lg w-full outline-none" />
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                      <div className="space-y-1">
                        ...
                        <input required type="number" step="0.01" placeholder="0,00" value={formVal} onChange={e => setFormVal(e.target.value)} className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl font-black text-2xl text-slate-800 focus:border-blue-500 outline-none" />
                      </div>
                  ...
                  <button type="submit" className={`w-full py-6 text-white font-black rounded-3xl uppercase shadow-2xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-4 text-sm mt-4 ${
                    transactionModal.convertingPlannedId ? 'bg-indigo-600' :
                    transactionModal.type === 'expense' ? 'bg-slate-900' : 
                    transactionModal.type === 'revenue' ? 'bg-emerald-600' : 'bg-blue-600'
                  }`}>
